# Mundial2026 Polla Web Application

## Descripción
Aplicación web **Vanilla JavaScript** diseñada para gestionar una "Polla Mundialista" del Mundial de Fútbol 2026. Los usuarios pueden registrar resultados reales del torneo, crear perfiles de participantes y gestionar múltiples apuestas con un sistema automático de puntuación.

La aplicación prioriza la claridad de los datos, la rapidez de respuesta y un diseño moderno/sobrio con soporte nativo para **Modo Oscuro**.

## Características Principales
- **Fase de Grupos**: Visualización de grupos, gestión de resultados de partidos y cálculo automático de tablas de posiciones (puntos, DG, GF, etc.).
- **Fases Eliminatorias**: Visualización de bracket (llaves) desde dieciseisavos hasta la final, con propagación automática de ganadores.
- **Gestión de Usuarios**: Registro de participantes (Nombre obligatorio, email/teléfono opcionales).
- **Sistema de Apuestas (CRUD)**: Los usuarios pueden crear múltiples pronósticos para todas las fases del torneo y posiciones finales (Campeón, Subcampeón, etc.).
- **Estadísticas**: Resumen de goles, rendimiento por grupos y ranking de goleadores/usuarios.
- **Modo de Prueba**: Soporte para entorno de test mediante el parámetro `?mode=test` en la URL para cargar datos experimentales sin afectar la producción.
...
├─ pages/          # vistas HTML
├─ init_data.js    # script de inicialización de JSON
└─ README.md
```

## Instalación
1. Clona o descarga el proyecto.
2. Ejecuta `npm install` para instalar la dependencia opcional **http-server**.
3. Ejecuta `npm run dev` para iniciar un servidor local (opcional, también puedes abrir los archivos directamente en el navegador).

## Uso
Abre `pages/index.html` (o cualquier página dentro de `pages/`) en el navegador. Los datos se guardarán automáticamente en la carpeta `data/`.

## Scripts
- `npm run dev` → inicia `http-server .` (servidor estático).
- `node init_data.js` → genera los archivos JSON vacíos necesarios.

## Contribuir
Asegúrate de seguir las guías de estilo y de ejecutar los test con `npm test` antes de enviar cambios.
...
Dieciseisavos
5 puntos por cada equipo acertado en esta fase.
Octavos
10 puntos por cada equipo acertado en esta fase.
Cuartos
20 puntos por cada equipo acertado en esta fase.
Semifinales
30 puntos por cada equipo acertado en esta fase.
Final
40 puntos por cada equipo acertado en esta fase.
Cuarto puesto
40 puntos por cada equipo acertado en esta fase.
Tercer puesto
50 puntos por cada equipo acertado en esta fase.
Subcampeón
60 puntos por cada equipo acertado en esta fase.
Campeón
70 puntos por cada equipo acertado en esta fase.
...
La data de estos archivos nunca reemplazarla o borrarla en procesos de compilacion del proyecto
Ejemplos:
users.json
[
  {
